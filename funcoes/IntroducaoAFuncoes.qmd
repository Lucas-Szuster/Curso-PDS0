...
::: {.callout-note title="Tente fazer! (clique para revelar a solução)" collapse="true"}
...
Isso significa que estamos copiando e colando o mesmo raciocínio várias vezes, mudando só um detalhe.
Ou seja, estamos fazendo trabalho repetido, manual e desnecessário.

E nos leva a duas reflexões:

#### 1. Estamos repetindo a mesma lógica várias vezes
...
#### 2. Se algo estivesse errado, teríamos que consertar em vários lugares
...
Esse tipo de repetição é uma grande fonte de bugs em programas reais.
Quanto mais partes iguais espalhadas pelo código, maior a chance de:
...
A programação oferece exatamente esse mecanismo:
a possibilidade de guardar um procedimento e reutilizá-lo sempre que necessário, mudando apenas a informação de entrada.
...
| Conceito                    | Descrição                                                                |
| --------------------------- | ------------------------------------------------------------------------ |
| **Parâmetro(s)** / **Entrada** | Valores que a função recebe como entrada, dados fornecidos para a função |
| **Processo**                | As instruções internas que a função executa com esses dados   |
| **Saída**                   | O que a função produz após executar seu processo              |
| **Retorno**                 | O valor que a função devolve (ou nada, se não houver retorno) |

## Funções que já conhecemos

Você já está usando funções no seu código, sem nem perceber! Três exemplos que você provavelmente usa o tempo todo em Python são as funções print(), input() e range(). Todas são essenciais para a interação com o usuário e a exibição de informações. No entanto, embora você já as tenha usado, talvez não tenha notado que ambas são funções completas, com entradas (valores que você passa para elas) e saídas (valores que elas retornam ou ações que realizam).
...
| Conceito          | Descrição                                                        |
| ----------------- | ---------------------------------------------------------------- |
| **Parâmetro(s)**  | O(s) valor(es) que você quer exibir (ex: uma string ou variável) |
| **Entrada**       | O que você quer mostrar na tela                                  |
| **Saída/Retorno** | Não retorna valor, apenas exibe a informação                     |
...
| Conceito          | Descrição                                                            |
| ----------------- | -------------------------------------------------------------------- |
| **Parâmetro(s)**  | O texto exibido para o usuário (ex: uma pergunta ou mensagem)        |
| **Entrada**       | O que o usuário digita como resposta                                 |
| **Saída/Retorno** | A função retorna uma **string** (mesmo que o usuário digite números) |

### Função range()

Você já está familiarizado com a função range(), uma das funções mais usadas em Python para controlar loops. Ela gera uma sequência de números, o que é perfeito quando queremos repetir uma ação várias vezes com controle preciso sobre os números.

#### Entradas: o que a função range() recebe

A função range() pode receber até três parâmetros:

* início: O número onde a sequência começa (inclusivo). O valor padrão é 0.

* fim: O número onde a sequência termina (exclusivo).

* passo: O valor que define o incremento entre os números. O valor padrão é 1.
...
#### Saída: o que a função range() retorna

A função range() retorna um objeto iterável que gera os números da sequência solicitada. Ele não cria uma lista de números, mas gera cada número conforme o loop vai sendo executado, o que é mais eficiente.

#### Resumo da função range()

| Conceito          | Descrição                                           |
| ----------------- | --------------------------------------------------- |
| **Parâmetro(s)**  | `início`, `fim`, e `passo` (todos são opcionais)    |
| **Entrada**       | A sequência de números que você deseja gerar        |
| **Saída/Retorno** | Um objeto iterável que gera os números da sequência |

## Sintaxe de utilização de funções
...
Imagine uma função que calcule a soma de dois números. Quais seriam as entradas e saídas dessa função? Quais são os possíveis tipos de dados que esta função aceita? 
...
| -------- | ------------------ |
| entradas | dois números: a, b |
| saídas   | a soma de a e b    |

a e b poderiam ser números inteiros ou floats, mas provavelmente não faria muito sentido eles serem diferentes disto.

:::

### Exercício 2

Como a definição de funções pode tornar a manutenção de um código mais fácil? Quais problemas podem surgir se não usarmos funções e acabarmos repetindo o mesmo código em várias partes do programa?

::: {.callout-tip collapse="true" title="Ver Solução"}

A definição de funções torna um codigo muito mais simples, pois se o código da função está errado, é necessário alterá-lo em somente um lugar, evitando com que partes estejam corretas e partes estejam incorretas. Caso não sejam utilizadas funções em código, é possível que a repetição de trechos de código leve a problemas que são praticmanete impossíveis de rastrear.

:::

### Exercício 3

Imagine que você tenha pontos 3D no espaço. A fórmula para calcular a distância entre dois destes pontos ($P1(x_1, y_1, z_1)$ e $P2(x_2, y_2, z_2)$) é a seguinte:

$$
d(P1, p2) = \sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2 + (z_2 - z_1)^2} 
$$

Como isto poderia ser adaptado para uma função (quais entradas e saídas)?

::: {.callout-tip collapse="true" title="Ver Solução"}

Para adaptar isto para função, é necessário somente que as entradas sejam os dois pontos ($P1$ e $P2$), pois com eles será feito o processamento de coordenadas para retornar, como saída, a distância entre eles.
