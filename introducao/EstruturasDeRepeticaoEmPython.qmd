...
Em algumas situações, é necessário interromper um loop antes que sua condição natural de parada seja atingida. Para isso, Python oferece o comando break.

O break encerra imediatamente a estrutura de repetição em que está inserido — seja um while ou um for. Após o break, o programa continua sua execução na primeira linha após o loop.

Esse comando é útil quando existe uma condição especial que, ao ser atendida, torna desnecessário continuar repetindo o bloco de código.
...
INÍCIO
    ENQUANTO condição FAÇA
        SE condição_de_parada_especial ENTÃO
            PARE
        FIM SE
        // bloco de instruções
    FIM ENQUANTO
...
#| eval: false
while condição:
    if condição_de_parada_especial:
...
O programa abaixo recebe números do usuário. Se o valor 999 for digitado, o loop é encerrado imediatamente — independentemente de outras condições.
...
#| eval: false
while True:
    # obtem um novo número do usuário
...
Como um while executa enquanto sua condição for verdadeira, escrever `while True` cria um loop cuja condição é sempre verdadeira, já que o valor `True` nunca se torna `False`. Isso gera um loop infinito, que só pode ser interrompido utilizando o comando break. Em outras palavras, sem o break, esse tipo de loop não teria um ponto natural de encerramento.
